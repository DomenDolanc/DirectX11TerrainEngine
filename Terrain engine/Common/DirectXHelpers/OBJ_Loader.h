...
namespace Terrain_engine
{
	...
	public:
		...
		bool LoadFile(std::string Path, std::string PathFolder)
		{
			...
			while (std::getline(file, curline))
			{
				...
				if (algorithm::firstToken(curline) == "f")
				{
					...
					{
						uint32_t indnum = (uint32_t)((Vertices.size()) - vVerts.size()) + iIndices[i];
						Indices.push_back(indnum);

						indnum = (uint32_t)((LoadedVertices.size()) - vVerts.size()) + iIndices[i];
				...
				if (algorithm::firstToken(curline) == "mtllib")
				{
					...
					pathtomat = PathFolder + pathtomat;
